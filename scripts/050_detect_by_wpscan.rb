...
def get_username site_url
  ...
  begin
    ...
    Rails.logger.error "== site is down"
    return nil
  end
...
def generate_password_dictionary(username)
  ...
  # 生成密码文件路径
  password_file = File.join(Rails.root, "tmp", "password_for_#{username}.txt")
  ...
  servers.find_in_batches(batch_size: 2000) do |batch|
    ...
    batch.each_with_index do |server, index|
      ...
      begin
        ...
        Rails.logger.error e.backtrace.join("\r")
        Rails.logger.error "== this website is down"
        next
      end
