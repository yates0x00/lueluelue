...
class FofaTool
    ...
    begin
      loop do
        ...
        unless status.success?
          Rails.logger.error "curl命令执行失败: #{stderr}"
          break
        end
      ...
      # 处理结果并保存到Server表
      process_and_save_results(results, domain, server)
  ...
  def process_and_save_results(results, main_domain, server = nil)
    ...
    results.each do |result|
      ...
      unless Server.exists?(name: clean_host)
        Server.create name: clean_host,
          ...
          level: 2,
          comment: "from FOFA subdomain scan of: #{main_domain}",
